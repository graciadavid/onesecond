  ...
  return (
    ...
    >
      <header className="flex items-center justify-between px-6 pt-12 pb-2 md:px-10 md:pt-8">
        ...
        <button onClick={handleShare} className="text-xs tracking-widest uppercase font-light hover:opacity-70 transition-opacity" style={{ color: tc }}>
          {copied ? "✓ Copied" : "Share"}
        </button>
      </header>
      ...
      <div className="hidden md:flex fixed right-6 top-1/2 -translate-y-1/2 flex-col gap-3">
        {prevBrand && (
          <Link href={`/${prevBrand.category}/${prevBrand.seoSlug}`}
            className="w-10 h-10 rounded-full flex items-center justify-center hover:opacity-80 transition-all"
            style={{ background: "rgba(255,255,255,0.15)", color: tc }}>↑</Link>
        )}
        {nextBrand && (
          <Link href={`/${nextBrand.category}/${nextBrand.seoSlug}`}
            className="w-10 h-10 rounded-full flex items-center justify-center hover:opacity-80 transition-all"
            style={{ background: "rgba(255,255,255,0.15)", color: tc }}>↓</Link>
        )}
      </div>
