 ...
 return (
   ...
   >
     ...
     <header className="flex items-center justify-between px-6 pt-12 pb-2 md:px-10 md:pt-8">
       ...
       <button
         onClick={handleShare}
         className="text-xs tracking-widest uppercase font-light hover:opacity-70 transition-opacity"
     ...
     {/* BLOQUE 1 — Nombre + absurdo — fondo del color de la marca */}
     <section className="flex flex-col items-center px-8 pt-8 pb-10 text-center gap-4">
       ...
       <div className="h-px w-24 mx-auto" style={{ background: tc, opacity: 0.4 }} />
       <p className="font-light leading-snug text-center max-w-xs mt-2"
         style={{ fontSize: "clamp(1rem, 3.5vw, 1.5rem)", fontFamily: "-apple-system, 'SF Pro Display', BlinkMacSystemFont, sans-serif", letterSpacing: "-0.02em", color: tc, opacity: 0.8 }}>
         <span className="tabular-nums font-thin">{absurdCount.toLocaleString("en-US")}</span>
         {" "}{brand.absurdUnit}
       </p>
     </section>

     {/* BLOQUE 2 — Since you arrived — fondo invertido */}
     <section
       className="flex-1 flex flex-col items-center justify-center px-8 py-12 text-center gap-4"
       style={{ background: tc }}
     >
     ...
     {/* Mobile swipe */}
     <div className="flex md:hidden flex-col items-center pb-16 pt-6 gap-1" style={{ background: tc }}>
       <p className="text-xs tracking-widest uppercase" style={{ color: brand.bgColor, opacity: 0.4 }}>swipe up</p>
       <p className="text-xs" style={{ color: brand.bgColor, opacity: 0.4 }}>↑</p>
       <p className="text-xs tracking-widest uppercase font-medium" style={{ color: brand.bgColor, opacity: 0.6 }}>{nextBrand.name}</p>
     </div>

     {/* Desktop buttons */}
     <div className="hidden md:flex items-center justify-center gap-4 pb-12 pt-6" style={{ background: tc }}>
       <Link
         ...
         style={{ border: `1px solid ${brand.bgColor}`, color: brand.bgColor, background: "transparent" }}
       >
         Next: {nextBrand.name} →
       </Link>
       <button
     ...
     <footer className="px-8 pb-6 text-center" style={{ background: tc }}>
       <p className="text-xs tracking-widest font-light" style={{ color: brand.bgColor, opacity: 0.3 }}>Data source: {brand.source}</p>
     </footer>
