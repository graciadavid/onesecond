...
export default function BrandPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const today = Math.floor((todayOffsetRef.current() + elapsed) * brand.perSecond)
  const thisYear = Math.floor((yearOffsetRef.current() + elapsed) * brand.perSecond)
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{ background: `linear-gradient(160deg, ${brand.bgColor}dd 0%, ${brand.bgColor} 100%)` }}>
      ...
      <section className="flex-1 flex flex-col justify-center px-6 md:px-16 py-6 md:py-8 max-w-4xl mx-auto w-full gap-8 md:gap-10">
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="text-white leading-snug font-light"
            style={{ fontSize: "clamp(1.4rem, 5vw, 2.8rem)", fontFamily: "-apple-system, 'SF Pro Display', BlinkMacSystemFont, sans-serif", letterSpacing: "-0.02em" }}>
        ...
        <div className="flex flex-col gap-5 md:gap-6">
          <div className="flex items-baseline justify-between gap-4">
            <p className="text-white/50 font-light shrink-0 text-sm md:text-base">{brand.unit} since you arrived</p>
            <p className="tabular-nums font-thin text-white text-right" style={{ fontSize: "clamp(1.6rem, 6vw, 3rem)", letterSpacing: "-0.03em", fontFamily: "-apple-system, 'SF Pro Display', BlinkMacSystemFont, sans-serif" }}>
              {fromEntry.toLocaleString("en-US")}
            </p>
          </div>

          <div className="flex items-baseline justify-between gap-4">
            <p className="text-white/50 font-light shrink-0 text-sm md:text-base">{brand.unit} today</p>
            <p className="tabular-nums font-thin text-white/60 text-right" style={{ fontSize: "clamp(1.6rem, 6vw, 3rem)", letterSpacing: "-0.03em", fontFamily: "-apple-system, 'SF Pro Display', BlinkMacSystemFont, sans-serif" }}>
              {today.toLocaleString("en-US")}
            </p>
          </div>

          <div className="flex items-baseline justify-between gap-4">
            <p className="text-white/50 font-light shrink-0 text-sm md:text-base">{brand.unit} this year</p>
            <p className="tabular-nums font-thin text-white/60 text-right" style={{ fontSize: "clamp(1.6rem, 6vw, 3rem)", letterSpacing: "-0.03em", fontFamily: "-apple-system, 'SF Pro Display', BlinkMacSystemFont, sans-serif" }}>
        ...
        <p className="text-white/20 text-xs font-light tracking-widest">Source: {brand.source}</p>
      ...
      <footer className="px-6 md:px-16 py-5 md:py-6 flex items-center justify-between">
        ...
        <button onClick={handleShare} className="px-5 py-2.5 rounded-full text-xs tracking-widest uppercase font-medium transition-all" style={{ background: "transparent", color: "rgba(255,255,255,0.3)" }}>
          {copied ? "✓ Copied" : "Share"}
        </button>
      </footer>
